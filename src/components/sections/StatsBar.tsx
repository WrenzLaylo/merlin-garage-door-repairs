      ...
      <div className="container-x grid gap-5 sm:grid-cols-4">
        {[
          ...
          <ScrollReveal key={stat.label} delay={index * 0.04}>
            <div className="group cursor-default rounded-2xl p-4 text-center transition-all duration-300 hover:-translate-y-1 hover:bg-brand-tint">
              <p className="font-display text-3xl font-bold text-ink transition-colors duration-300 group-hover:text-brand-deep">
                <AnimatedCounter value={stat.value} suffix={stat.suffix} decimals={stat.decimals ?? 0} />
              </p>
              <p className="mt-1 text-sm text-muted transition-colors duration-300 group-hover:text-brand-deep">{stat.label}</p>
            </div>
          </ScrollReveal>
        ))}
      </div>
